...
public partial class ClickHouseMigratorTests : ClickHouseFacadesTestsCore
{
	...
	public async Task OneMigrationToApply_ApplyMigrations_MigrationApplied()
	{
		...
		Assert.AreEqual(
			$"insert into {databaseName}.{historyTableName} values "
	...
	[TestMethod]
	public async Task SpecifiedMigrationsTableCluster_ApplyMigrations_MigrationsTableCreatedOnCluster()
	{
		const string databaseName = "test";
		const string historyTableName = "db_migrations_history_on_cluster";
		const string clusterName = "test_cluster";

		SetupMigrations(databaseName, historyTableName, clusterName, rollbackOnMigrationFail: false);
		SetupAppliedMigrations(databaseName, historyTableName);
	...
	private void SetupAppliedMigrations(
		string databaseName,
		string historyTableName,
		params AppliedMigration[] appliedMigrations)
	{
		MockExecuteReader<ClickHouseMigrationContext, AppliedMigration>(
			sql => sql == $"select id, name from {databaseName}.{historyTableName} final",
			appliedMigrations,
